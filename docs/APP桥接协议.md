...
### 2.1 `vision_capabilities` 命令

`vision_capabilities` 是向后兼容的只读查询，不改变协议版本。APP 在创建视觉巡检任务前应先调用它，获取当前**已加载且激活**的 YOLO 模型及其完整类别表：

```json
{"id":"vision-cap-1","cmd":"vision_capabilities"}
```

成功响应的 `data.capabilities` 示例：
  ...
  "models": [
    {
      ...
      "labels": ["person", "bicycle", "car", "bottle"]
    }
  ],
...
- `name` 是路线任务 `local_model` 应填写的模型注册名，不是文件路径。
- `labels` 是模型实际输出的完整类别名；APP 应以此生成本地识别目标的下拉列表。
- 视觉节点尚未启动、模型加载失败或 APP Bridge 尚未收到能力消息时，`capabilities` 为 `null`；APP 应提示模型不可用，而不是猜测类别。
- `vision` 遥测通道也会在能力信息到达或更新时推送 `{ "capabilities": ... }`。

路线仍允许 APP 提交不在 `labels` 中的 `target`。这类任务必须设置 `use_vlm_fallback: true`：本地 YOLO 不会确认目标存在，巡检节点会把捕获图像交给已配置的远程 VLM；若 VLM 未配置或无法给出结构化结果，结论为 `UNKNOWN`，不能视为“不存在”。

```json
{
  "task_id": "check-extinguisher",
